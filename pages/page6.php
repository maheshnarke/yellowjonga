...
<header>
    <div id="p3hero" style="background-image: url(img/contact.png);background-size: 100% 100%;background-position: center;background-clip: border-box;margin-top: 4%; position:relative;height: 450px;background-repeat: no-repeat">
        ...
        <div class="contheader">We would love to hear from you!</div>
    </div>
...
<div class="contwrap">
    ...
    if(isset($_SESSION['user']['username'])){
        ...
        $query ="SELECT username, email, contact_num FROM members WHERE username='".$_SESSION['user']['username']."'";
        $result = $conn->query($query);
        while ($row = $result->fetch_assoc()) {

            $uname=$row["username"];
            $uemail=$row["email"];
            $ucontact=$row["contact_num"];
    ...
    <div class="contmain">
        <input class="continput" type="text" name="textName" value="<?php echo $uname;?>" id="textName" placeholder="Name"/>

        <input class="continput" type="text" name="textEmail" value="<?php echo $uemail;?>" id="textEmail" placeholder="Email"/>

        <input class="continput" type="text" name="textPhone" value="<?php echo $ucontact;?>" id="textPhone" placeholder="Phone number (optional)" maxlength="10"/>
